...
dota_smooth = []
for d in dota:
    dota_smooth.append(savgol_filter(d, 51, 3))
plot_data(dota[13])    
dota = dota_smooth
...
def save_btn():
    ...
    for i in range(len(dota[0])):
        ...
        mesh.write(f"C:\\Users\\ameen\\OneDrive\\Desktop\\EEG_Data\\EEG_{i}.ply")
    sv_btn.switch()
...
sl = plot.addSlider2D(slider1, 0, len(dota[0])-1, value=0,
               pos="bottom-right", title="Window Number",c='w')
...
# intpr = RBF_Interpolation(mesh,pts,[j[0] for j in dota])
# mesh.cmap('jet', intpr, vmin=-vmax, vmax=vmax)
# scl = mesh.addScalarBar()
# for i in range(t1,t2):
#     print(i)
#     intpr = RBF_Interpolation(mesh,pts,[j[i] for j in dota])
#     datas = [l[i] for l in dota]
#     points.cmap('jet', datas, vmin=-vmax, vmax=vmax)
#     mesh.cmap('jet', intpr, vmin=-vmax, vmax=vmax)
    
    
    
    
#     plot.show(mesh,points,scl)

...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.verticalLayoutWidget.setGeometry(QtCore.QRect(10, 20, 281, 550))
        ...
        self.gridLayoutWidget.setGeometry(QtCore.QRect(300, 20, 1075, 750))
        ...
        self.horizontalLayoutWidget.setGeometry(QtCore.QRect(400, 770, 641, 80))
        ...
        # Button 9
        self.plot2D = QtWidgets.QPushButton()
        self.plot2D.setObjectName("plotSmooth")
        ...
        self.plot2D.clicked.connect(self.plot2DScalp)
    ...
    def plot2DScalp(self):
        fig = self.win4.figure()
        ax = fig.add_subplot(111, aspect = 1)
        self.tabWidget.addTab(self.vtkWidget,'2D Scalp Plot')
        self.win4.clear()
        l_freq = self.doubleSpinBox.value()
        h_freq = self.doubleSpinBox_2.value()
        data = get_power_values(self.raw.copy().filter(l_freq,h_freq).get_data()[:len(self.raw.get_data())-1],300)
        sensors = get_sensors_from_montage(self.raw.info['ch_names'])
        ...
        head = matplotlib.patches.Ellipse(xy = centroid, height = 1,width=.9,angle=0, edgecolor = "k", facecolor = "none",zorder=2)
        whitearea = matplotlib.patches.Ellipse(xy = centroid, height = 1.15,width=1.05,angle=0, edgecolor = "white", facecolor = "none",linewidth=30,zorder=1)
        ...
        self.win4.scatter(x, y, marker = 'o', c = 'b', s = 15, zorder = 3)

        axfreq = self.win4.axes([0.25, 0.1, 0.65, 0.03])
        vmax = max([max(i[0:len(data)]) for i in data])
        vmin = min([min(i[0:len(data)]) for i in data])
    ...
    def updateGraph(self):
        ...
        self.powerValues = get_power_values(data,300)
        ...
        self.vmin = min([min(i[0:len(self.smooth_data)]) for i in self.smooth_data])
        self.mesh.cmap('jet',intpr,vmin= self.vmin, vmax=self.vmax)
    ...
    def plot3D(self):
        ...
        if(self.obj_path != ''):
            ...
            # self.tabWidget.addTab(self.vtkWidget,'3D Plot')
            # self.vp = Plotter(qtWidget=self.vtkWidget)
            self.vp.show(self.mesh,*self.txts, self.sensors_pts,self.filename_txt,interactive=1).addSlider2D(self.slider,0,len(self.smooth_data[0])-1,value=0,pos="buttom-right",title="Window Number",c='k')
        # LATER TODO
        # else:
        #     self.updateGraph()
    ...
    def getEDFFile(self):
        ...
        if self.path != '':
            ...
            self.verticalLayout.addWidget(self.plot2D)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.plot2D.setText(_translate("MainWindow", "Plot 2D Scalp"))
